...
public class MixtureTextView extends RelativeLayout {
    ...
    //转换像素
    private int dip2px = 10;
    public MixtureTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);
    }
    public MixtureTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context,attrs);
    }
    private void init(Context context, AttributeSet attrs) {
        readAttrs(context, attrs);
//        dip2PX(context,dip2px);
        //just for text
        if (mText == null) {
            mText = getResources().getString(R.string.unknown);
        }
        //get text
    ...
    private List<Rect> caculateViewYBetween(int y1, int y2) {
        ...
        if (rs.size() >= 2) {
            ...
            for (int i = 1; i < rs.size(); i++) {
                Log.e("TAG_舉行hi否相交", (Rect.intersects(pre, next)) + "");
                //if相交
                if (Rect.intersects(pre, next)) {
                    int left1 = Math.min(pre.left, next.left);
                    int right1 = Math.max(pre.right, next.right);
                    Log.e("TAG_舉行hi否相交", "left1=" + left1 + ";right=" + right1);
                    res.remove(pre);
                    res.remove(next);

                    left = pre.left+ pre.width()+dip2px;
                    right = pre.right + pre.width()+dip2px;
                    res.add(new Rect(left, y1, right, y2));

                    Log.e("TAG_舉行hi否相交", "left=" + left + "right=" + right);
                    if (res.size() >= 2) {
                        pre = rs.get(0);
                        next = rs.get(1);
                    } else {
